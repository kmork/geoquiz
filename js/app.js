...
function resetDeck(){
  deck = shuffle([...DATA]); // copy + shuffle
  deckIndex = 0;
}
...
function nextQ(){
  if(deck.length === 0 || deckIndex >= deck.length){
    ...
    elStatus.innerHTML = `You’ve completed all countries.<br>Final score: <b>${score}</b> in <b>${rounds}</b> rounds.`;
    ...
    drawMap("Norway", false); // or just clear map / keep last one
    return;
  }
