...
**Visual clues** are interactive evidence items available from Skyline case 1 with a ~40% chance at specific investigation locations. Three types exist: **scramble** (airport) presents the destination capital's letters jumbled on a boarding pass for the player to rearrange via drag-and-drop; **outline** (library) shows the destination country's silhouette on a torn atlas page without labels; **flag** (market) displays a torn fragment of the destination country's flag with a jagged clip-path revealing roughly half, expanding slightly on hover. Visual clues are mixed into each location's normal Skyline clue priority rather than checked before ordinary clues, so they add variety without crowding out route-deduction clues. All visual clues use the "Examine Evidence" button pattern, rendering inside a themed evidence card with a dismiss button. When an outline or flag fragment is examined, a compact copy of that visual evidence is added to the Dossier for later review; scramble clues add the decoded capital after the player solves them. They are tracked by the `usedClueIds` deduplication system. The debug command "clues" forces visual clue generation first at eligible locations.

Skyline clue ordering is case-tuned inside the route provider. Cases 1-3 favor high-signal distance, continent, capital-initial, hemisphere/latitude, and cargo clues. Cases 4-6 bring legal clock shifts and gateway reasoning forward while keeping direct geography available. Cases 7-9 lean harder on gateway, hemisphere, distance, and manifest-relevant clues. Case 10 prioritizes the same dimensions used by the final manifest proof: distance band, gateway band, legal clock shift, hemisphere, and continent. This tuning changes clue priority only; it does not change route generation, scoring, investigation counts, or the Manifest Board.
