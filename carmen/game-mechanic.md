...
The current curated prototype is **The Renaissance Wave**. It opens with the Mona Lisa stolen from the Louvre in Paris, then reframes the case as an escalating strike list of famous Renaissance paintings. The fiction is no longer just single-object provenance laundering: the thief is assembling a prestige route through major Renaissance hubs, altering catalogue cards, chapel access notes, reserve indexes, shipping sleeves, and museum comparison files so the theft wave looks like one inevitable canon.
...
Each city scene lets the player investigate Airport, Hotel, Market, Library, and Embassy sources freely. World Case Files can now also author scene-specific puzzle locations inside a chapter, such as a dispatch ledger, chapel layout, or rehanging plan. The Investigate tab reuses the OpenStreetMap-compatible city map and POI placement from City Map Open Case, with an added curated scene marker when the case supplies exact coordinates. Evidence is curated per case rather than generated from OSM truth. Evidence can be route-oriented, cultural, confirmatory, or puzzle-based. Cultural evidence may reference art history, museum provenance, exports, language, old collection routes, or historic trade context. Puzzle evidence should stay grounded in authored museum material rather than abstract game logic. OSM and city map data remain placement/context only, not clue truth.

The world map always stays open as the main trail surface. It uses subtle atlas guidance instead of a large travel desk. Countries remain selectable, but the map only gives light visual help about how collected evidence fits:
...
The map overlay is intentionally compact: a tiny confidence summary, a small hint toggle, and one short atlas-hint line that explains what a clue means in world terms (for example, export clues pointing toward coffee producers) without listing answers. Detailed evidence comparison now appears only when the player inspects a country on the map. In the current experiment, clicking a non-current country opens a second step on the same atlas: the map zooms into that country and shows selectable city destinations using the curated POI/capital data. The player commits travel by choosing a city, and the airplane animation now flies to that specific city point. Every arrival is treated as a real trail stop, even when it does not advance the case. A wrong country still behaves like a normal dead end. A right country with the wrong city produces a softer in-country dead end so the player can back out and try another city without advancing the case. The next flight always starts from the most recent landed stop, not from the last authored chapter city. The player can backtrack to previous cities; some follow-up evidence can unlock after later travel.

Advancing to the next city requires more than choosing the right destination. The player needs enough evidence on the board: route evidence, cultural-pattern evidence, and confirmation/contradiction evidence, plus puzzle evidence when the current scene authors it as required. Cases can also author multi-scene country chapters. In that structure, atlas travel moves between countries while local scene progression happens inside the current country chapter. Once the active scene or chapter is ready, the evidence review happens in the atlas inspection panel and on the board rather than in a separate overlay. This keeps the mode slower and more detective-like than the chase modes without interrupting the map flow.

The status-bar gauge surfaces the unlock predicate so it can never disagree with `canSubmitProof`. It has five segments — Route, Culture, Confirm, Puzzle, Proof. Route, Culture, and Confirm read `complete` when at least one corroborated clue of that category is on the board, `partial` when only a preliminary (vague-witness) clue exists, and `incomplete` otherwise. Puzzle reads `not_required`, `complete`, or `incomplete` depending on whether the active scene authors a puzzle and whether it is solved. Proof shows `corroboratedCount / max(3, stop.minimumEvidence)` and is `complete` only when the threshold is met. When all required segments are `complete`, `evidenceReady` is true and the clock highlights the ready state; preliminary clues never satisfy the unlock until they are corroborated.

Multi-scene chapters can now expose a local trail state. The current prototype uses that to keep the travel board visible inside the Italy chapter as a short rail-style line between Milan, Florence, and Rome rather than treating those scenes as silent internal jumps. This is intentionally still a light presentation layer, not yet a full city-to-city route system.

The case card artifact view now doubles as a case-evidence panel. When a case defines richer work metadata, the panel can show the active painting, institution, current objective, escalation summary, and a list of prepared evidence-image assets for that scene.
