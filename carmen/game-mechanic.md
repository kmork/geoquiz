...
**Visual clues** are interactive evidence items that appear from case 3 onward with a ~40% chance at specific investigation locations. Three types exist: **scramble** (airport, case 3+) presents the destination capital's letters jumbled on a boarding pass for the player to rearrange via drag-and-drop; **outline** (library, case 3+) shows the destination country's silhouette on a torn atlas page without labels; **flag** (market, case 4+) displays a torn fragment of the destination country's flag with a jagged clip-path revealing roughly half, expanding slightly on hover. All visual clues use the "Examine Evidence" button pattern, rendering inside a themed evidence card with a dismiss button. They are tracked by the `usedClueIds` deduplication system. The debug command "clues" forces 100% visual clue generation at eligible locations.
...
Each Skyline case is assigned a specific pattern type via `CASE_PATTERN_TYPES` in the route provider. Route generation preferentially selects routes that satisfy the assigned pattern. The assignments are: cases 1 and 4 and 10 use same-continent, cases 2 and 8 use same-hemisphere, cases 3 and 6 use distance-band, cases 5 and 9 use clock-drift, and case 7 uses gateway-band. This ensures the Dispatcher's signature varies across the campaign while every case reliably has a pattern to discover.
